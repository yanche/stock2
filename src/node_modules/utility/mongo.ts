...
export class CollClient {
    private _colhub: mods.Hub<mongodb.Collection>;
    private _deffields: Object;

    getOne(filter: Object, fields?: Object): bb<any> {
        return this._colhub.get()
            .then(col => new bb<any>((res, rej) => {
                col.findOne(filter, { fields: fields || this._deffields }, (err: Error, doc: any) => {
                    if (err != null) rej(err);
                    else res(doc);
                })
            }));
    }
    getAll(filter: Object, fields?: Object, orderby?: Object): bb<Array<any>> {
        return this._colhub.get()
            .then(col => new bb<any>((res, rej) => {
                var cursor = col.find(filter, fields || this._deffields);
                if (orderby != null) cursor = cursor.sort(orderby);
                cursor.toArray((err: Error, docs: Array<any>) => {
                    if (err != null) rej(err);
                    else res(docs);
                })
            }));
    }
    getMul(filter: Object, fields: Object, orderby: Object, skip: number, take: number): bb<Array<any>> {
        return this._colhub.get()
            .then(col => new bb<any>((res, rej) => {
                var cursor = col.find(filter, fields || this._deffields);
                if (orderby != null) cursor = cursor.sort(orderby);
                cursor.limit(take + skip).skip(skip).limit(take).toArray((err: Error, docs: Array<any>) => {
                    if (err != null) rej(err);
                    else res(docs);
                })
            }));
    ...
    updateAll(filter: Object, update: Object, upsert: boolean): bb<void> {
        return this._colhub.get()
    ...
    removeAll(filter: Object): bb<void> {
        return this._colhub.get()
            .then(col => new bb<void>((res, rej) => {
    ...
    findAndModify(filter: Object, update: Object, fields: Object, returnnew: boolean, upsert: boolean): bb<any> {
        return this._colhub.get()
            .then(col => new bb<any>((res, rej) => {
    ...
    bulkInsert(arr: Array<any>): bb<void> {
        return this._colhub.get()
            .then(col => new bb<void>((res, rej) => {
                const bulk = col.initializeUnorderedBulkOp();
...
export class DbClient {
    ...
    getCollClient(collname: string, deffields: Object): CollClient {
        return new CollClient(this._dbhub, collname, deffields);
    }
...
function convObjId(id: any): mongodb.ObjectID {
    if (id == null)
        return null;
    else {
        try {
