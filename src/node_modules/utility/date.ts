...
export function dateTs2DateKey(datets: number): string {
    const date = new Date(datets * dateMS);
    return `${put0PadString(date.getUTCFullYear().toString(), 4)}-${put0PadString(date.getUTCMonth().toString(), 2)}-${put0PadString(date.getUTCDate().toString(), 2)}`;
}
