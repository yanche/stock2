...
export function genGetMul<T>(resx: string) {
    return function (filter: Object, fields: Object, page: number, pageSize: number, orderby: Object) {
...
export function genUpsert(resx: string) {
    return function (filter: Object, update: Object) {
        return expectJson<{ insertedId: string }>('POST', resx, { filter: filter, update: update }, 0, { verb: 'UPSERT' });
    }
}

export function genBulkUpdate(resx: string) {
    return function (list: Array<{ filter: Object, update: Object }>) {
        return expectRaw('POST', resx, { list: list }, 0, { verb: 'BULKUPDATE' }).then(() => null);
    }
}

export function genBulkUpsert(resx: string, upsert: boolean, pagesize: number) {
