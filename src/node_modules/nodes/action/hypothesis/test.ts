...
export interface HypoTestInput {
    ...
    cpDefRef: DefRef;
    cpoutDefRef: { [key: string]: DefRef };
    envDef: { [key: string]: { type: string, pack: any } };
}
            ...
            .then(data => { //rdp: dp for revenue calculation
                ...
                return utility.whileLoop(() => bb.resolve(finished < total), () => {
                    ...
                    return bb.all([datapvd.literal.resolve(utility.refReplace(input.cpDefRef, p)), resolveDPs(input.cpoutDefRef, p), resolveDPs(input.envDef)])
                        .then(data => {
                            const mindayts = rdp.remoteTs(rdp.minTs, 20); //上市前20天忽略
                    ...
                    .then(() => {
                        ...
                        for (let dpoutname in input.cpoutDefRef) {
                            ...
                            if (input.header) allcontent = reportHeaders(input.penum, input.envDef) + '\r\n' + allcontent;
...
function reportHeaders(params: { [key: string]: any }, envdefs: { [key: string]: any }) {
...
function resolveDPs(dpl: { [key: string]: any }, repl?: { [key: string]: any }) {
    const prms: Array<bb<{ name: string, dp: datapvd.def.DataPvd<any> }>> = [];
    for (let i in dpl) {
        const v = dpl[i];
        prms.push(datapvd.literal.resolve(<datapvd.literal.LiteralDP>(repl == null ? v : utility.refReplace(v, p))).then(pvd => {
