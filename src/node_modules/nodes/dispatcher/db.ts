...
const task = dbc.getCollClient(config.dbCols.task, taskFields);
const simulate = dbc.getCollClient(config.dbCols.simulate, simulateFields);
const simtrack = dbc.getCollClient(config.dbCols.simtrack, simtrackFields);
const rtplan = dbc.getCollClient(config.dbCols.rtplan, rtplanFields);
const rtprog = dbc.getCollClient(config.dbCols.rtprog, rtprogFields);
const rtprogout = dbc.getCollClient(config.dbCols.rtprogout, rtprogOutFields);
const rtprice = dbc.getCollClient(config.dbCols.rtprice, rtpriceFields);
const systrack = dbc.getCollClient(config.dbCols.systrack, systrackFields);
