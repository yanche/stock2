...
function mmts(pack: OffsetFacPack, dp: def.DataPvd<any>) {
    ...
    else if (pack.N > 0) return { minTs: dp.minTs, maxTs: dp.remoteTs(dp.maxTs, -pack.N) || facutil.dateTsOffset(dp.minTs, -1) };
    else return { maxTs: dp.maxTs, minTs: dp.remoteTs(dp.minTs, -pack.N) || facutil.dateTsOffset(dp.maxTs, 1) };
}
...
const offsetFac: IFactory<OffsetFacPack, any> = facutil.dpTransform<OffsetFacPack, any, OffsetRet>({
    ...
    gen: (pack: OffsetFacPack, dp: def.DataPvd<any>, ts: number): bb<OffsetRet> => {
        const bts = dp.remoteTs(ts, pack.N);
        return dp.get(bts).then(v => { return { val: v, ts: bts } });
    },
    mmts: mmts
});
...
const offsetVFac: IFactory<OffsetFacPack, any> = facutil.dpTransform<OffsetFacPack, any, any>({
    ...
    gen: (pack: OffsetFacPack, dp: def.DataPvd<any>, ts: number): bb<any> => {
        return dp.get(dp.remoteTs(ts, pack.N));
    },
    mmts: mmts
});
